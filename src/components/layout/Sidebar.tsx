...
export default function Sidebar({ isOpen, collapsed, onToggleCollapse, onCloseMobile }: SidebarProps) {
	const baseClasses = `fixed inset-y-0 left-0 z-40 flex flex-col bg-zinc-900 border-r border-zinc-800 transition-transform duration-200`
	...
	return (
		<aside className={`${baseClasses} ${mobileClass} lg:relative lg:translate-x-0 ${widthClass}`}>
			...
			<div className="flex items-center justify-between px-3 py-3 border-b border-zinc-800">
				{!collapsed && (
					<span className="font-bold text-zinc-100 text-sm truncate">BDD Lab</span>
				)}
				<button
					className="hidden lg:flex items-center justify-center text-zinc-400 hover:text-zinc-100 transition-colors ml-auto"
